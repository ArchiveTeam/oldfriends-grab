...
LUA_SHA1 = get_hash(os.path.join(CWD, 'oldfriends.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--timeout", "30",
            "--tries", "inf",
            "--domains", "oldfriends.co.nz",
            "--span-hosts",
            "--waitretry", "30",
            "--warc-file", ItemInterpolation("%(item_dir)s/%(warc_file_base)s"),
            "--warc-header", "operator: Archive Team",
            "--warc-header", "oldfriends-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("oldfriends-user: %(item_name)s"),
        ]
...
project = Project(
    ...
    """,
    utc_deadline=datetime.datetime(2016, 01, 15, 23, 59, 0)
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="oldfriends"),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
